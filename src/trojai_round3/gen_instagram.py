...
def forensic(args, preeval=True):
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 10 == 0 and Print_Level > 0:
            ...
            save_path = f'forensics/trojai_instagram/{args.filter_type}_{args.model_id}'
...
if __name__ == '__main__':
    ...
    # ['LomoFilterXForm', 'NashvilleFilterXForm', 'GothamFilterXForm', 'KelvinFilterXForm', 'ToasterXForm']
    parser.add_argument('--filter_type', default='LomoFilterXForm', help='filter type')
    ...
    # Take 1 model for example
    count = 1

    folder = 'forensic/trojai_instagram/'
    if not os.path.exists(folder):
        os.makedirs(folder)

    for i in range(total):
        model_idx = 'id-' + str(i).zfill(8)
        model_architecture = df.loc[model_idx]['model_architecture']
        filter_type = df.loc[model_idx]['instagram_filter_type']
        # filter_type = df.loc[model_idx]['trigger_type_option']

        if filter_type != args.filter_type:
            continue
        if count == 0:
            break
        ...
        args.victims = []
        for i in victims:
            args.victims.append(int(i))

        args.model_filepath = '/data/share/trojai/trojai-round3-dataset/{}/model.pt'.format(model_idx)
        args.clean_dirpath = '/data/share/trojai/trojai-round3-dataset/{}/clean_example_data/'.format(model_idx)
        args.poison_dirpath = '/data/share/trojai/trojai-round3-dataset/{}/poisoned_example_data/'.format(model_idx)

        args.ground_truth = bool(df.loc[model_idx]['poisoned'])
