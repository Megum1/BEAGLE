...
def filter_img(h=224, w=224, Troj_size=1600):
    mask = np.zeros((h, w), dtype=np.float32)
...
def load_filter_forensics():
    ...
    id_list = [[21], [0, 28, 63], [41, 53, 58], [44, 55, 76], [68, 79]]
    ...
    for i in range(len(filter_list)):
        ...
        for j in id_list[i]:
            ...
            save_id = filter_id + '_' + model_id
            weights, bias = pickle.load(open(f'forensics/trojai_instagram/{save_id}/linear_param', 'rb'), encoding='bytes')
            _w.append(weights)
            _b.append(bias)
