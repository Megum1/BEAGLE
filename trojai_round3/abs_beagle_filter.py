...
from synthesis_scanner import load_filter_forensics
...
def loss_fn(inner_outputs_b, inner_outputs_a, logits, batch_labels, con_mask, neuron_mask, label_mask, base_label_mask, acc, e, re_epochs, ctask_batch_size):
    ...
    #     # TODO: if finally mask_nz > 1, drop the result
    #     mask_loss = torch.sum(torch.gt(con_mask[i], mask_epsilon))

    #     mask_nz = torch.sum(torch.gt(con_mask[i], mask_epsilon))
    #     mask_cond = torch.gt(mask_nz, 1.1)
    #     mask_add_loss += torch.where(mask_cond, re_mask_weight * mask_loss, 0. * mask_loss)

    mask_loss = torch.sum(torch.gt(con_mask, mask_epsilon))
    mask_nz = torch.sum(torch.gt(con_mask, mask_epsilon))
    mask_add_loss = 0. * mask_loss
    ##########################################################################################
...
def reverse_engineer(model_type, model, children, oimages, olabels, weights_file, Troj_Layer, Troj_Neurons, samp_labels, base_labels, Troj_size, re_epochs, num_classes, n_re_imgs_per_label, n_neurons, ctask_batch_size):
    ...
    mask = np.random.random((1, 5, 1, 1, 1))
    mask = np.tile(mask, (ctask_batch_size, 1, 1, 1, 1))
    mask = torch.FloatTensor(mask).cuda()

    # Useless
    delta = torch.zeros((ctask_batch_size, 5, 1, 1, 1)).cuda()

    mask.requires_grad = True
    optimizer = torch.optim.Adam([mask], lr=re_mask_lr, betas=(0.5, 0.9))
    ...
    for e in range(re_epochs):
        ...
        for i in range( math.ceil(float(len(images))/re_batch_size) ):
            ...
            con_mask = F.softmax(mask / temperature, dim=1)
            batch_mask  = torch.tensordot(batch_v2d, con_mask,  ([1], [0]))
            ...
            height = 224
            weights = (batch_mask * forensic_weights.cuda()[None, ...]).sum(dim=1)
            bias = (batch_mask * forensic_bias.cuda()[None, ...]).sum(dim=1)
            ...
            r = torch.sum(batch_data * weights[:, 0:3, :, :] + bias[:, 0:1, :, :], dim=1, keepdim=True)
            g = torch.sum(batch_data * weights[:, 3:6, :, :] + bias[:, 1:2, :, :], dim=1, keepdim=True)
            b = torch.sum(batch_data * weights[:, 6:9, :, :] + bias[:, 2:3, :, :], dim=1, keepdim=True)
            output = torch.cat([r, g, b], dim=1)
            in_data = torch.clamp(output, 0., 1.)
            ...
            loss, vloss1, vloss2, vloss3, relu_loss1, relu_loss2, mask_loss, mask_nz, mask_add_loss, logits_loss\
                    = loss_fn(inner_outputs_b, inner_outputs_a, logits, batch_labels, con_mask, batch_neuron_mask, batch_label_mask, batch_base_label_mask, facc, e, re_epochs, ctask_batch_size)
            if e > 0:
    ...
    con_mask = con_mask.cpu().detach().numpy()

    # cleaning up
    for handle in handles:
        handle.remove()

    return faccs, delta, con_mask, optz_labels
...
def re_mask(model_type, model, neuron_dict, children, images, labels, n_neurons_dict, scratch_dirpath, re_epochs, num_classes, n_re_imgs_per_label):
    ...
    for key in sorted(neuron_dict.keys()):
        ...
        for task_i in range(math.ceil(float(n_tasks)/task_batch_size)):
            ...
            for task_j in range(ctask_batch_size):
                if max_acc[task_j] >= reasr_bound:
                    validated_results.append( max_results[task_j] )
        return validated_results


def stamp(n_img, mask):
    # kdim = 4
    # delta: (1, 9, kdim, kdim)
    # mask:  (1, 9, kdim, kdim)
    height = 224
    inputs = torch.FloatTensor(n_img.copy())
    weights = (torch.FloatTensor(mask) * forensic_weights[None, ...]).sum(dim=1)
    bias = (torch.FloatTensor(mask) * forensic_bias[None, ...]).sum(dim=1)

    weights = F.upsample(weights, size=height, mode='bicubic', align_corners=True)
    bias    = F.upsample(bias   , size=height, mode='bicubic', align_corners=True)

    r = torch.sum(inputs * weights[:, 0:3, :, :] + bias[:, 0:1, :, :], dim=1, keepdim=True)
    g = torch.sum(inputs * weights[:, 3:6, :, :] + bias[:, 1:2, :, :], dim=1, keepdim=True)
    b = torch.sum(inputs * weights[:, 6:9, :, :] + bias[:, 2:3, :, :], dim=1, keepdim=True)
    output = torch.cat([r, g, b], dim=1)
...
def prune_by_neuron_mask(fxs, fys, fxs2, base_label, optz_label, model, children, sample_layers):
    inner_info = {}
...
def test(model, model_type, test_xs, test_ys, children, target_layers, sample_layers, result, scratch_dirpath, num_classes):
    ...
    cond = (rmask > mask_epsilon).reshape(-1)
    forensic_delta = np.asarray([1.0, 1.0, 0.85, 0.9, 0.9])
    forensic_delta = np.sum(forensic_delta * cond)
    rmask = rmask * (rmask > mask_epsilon)

    t_images = stamp(clean_images, rmask)
    t_images = t_images * forensic_delta + clean_images * (1 - forensic_delta)
    t_images = np.clip(t_images, 0., 1.)
    ...
    tlabel = optz_label
    if best_tscore > 0.99:
        prune_flag = prune_by_neuron_mask(test_xs, test_ys, t_images, best_base_label, tlabel, model, children, sample_layers)
        if prune_flag:
            best_tscore = 0
    ...
    if len(results) > 0:
        ...
        for result in results:
            ...
            with open(RE_mask, 'wb') as f:
                pickle.dump(rmask, f)
            reasr_info.append(['{:.2f}'.format(reasr), '{:.2f}'.format(reasr_per_label), mode, str(optz_label), str(samp_label), str(base_label), RE_img, RE_mask, RE_delta, np.sum(rmask), acc, '{:.2f}'.format(inner_diff1), '{:.2f}'.format(inner_diff2), label_results_str])
...
if __name__ == "__main__":
    ...
    parser.add_argument('--model_filepath', default=None, help='model file path')
    parser.add_argument('--result_filepath', default='output', help='result file path')
    parser.add_argument('--scratch_dirpath', default='scratch', help='scratch directory path')
    ...
    # TODO: Global log file to keep track of detection results
    logfile = 'result.txt'

    # Load the forensics results
    # (5, 9, 4, 4), (5, 9, 4, 4)
    forensic_weights, forensic_bias = load_filter_forensics()

    for i in range(total):
        model_id = 'id-' + str(i).zfill(8)
        args.model_filepath = f'{args.dataset_dir}/{model_id}/model.pt'
        args.examples_dirpath = f'{args.dataset_dir}/{model_id}/clean_example_data/'

        # Designed for polygon triggers
        trigger_type = df.loc[model_id]['trigger_type']
