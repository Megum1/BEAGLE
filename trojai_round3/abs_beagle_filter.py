...
def reverse_engineer(model_type, model, children, oimages, olabels, weights_file, Troj_Layer, Troj_Neurons, samp_labels, base_labels, Troj_size, re_epochs, num_classes, n_re_imgs_per_label, n_neurons, ctask_batch_size):
    ...
    for e in range(re_epochs):
        ...
        for i in range( math.ceil(float(len(images))/re_batch_size) ):
            ...
            r = torch.sum(batch_data * weights[:, 0:3, :, :] + bias[:, 0:3, :, :], dim=1, keepdim=True)
            g = torch.sum(batch_data * weights[:, 3:6, :, :] + bias[:, 3:6, :, :], dim=1, keepdim=True)
            b = torch.sum(batch_data * weights[:, 6:9, :, :] + bias[:, 6:9, :, :], dim=1, keepdim=True)
...
def stamp(n_img, mask):
    ...
    r = torch.sum(inputs * weights[:, 0:3, :, :] + bias[:, 0:3, :, :], dim=1, keepdim=True)
    g = torch.sum(inputs * weights[:, 3:6, :, :] + bias[:, 3:6, :, :], dim=1, keepdim=True)
    b = torch.sum(inputs * weights[:, 6:9, :, :] + bias[:, 6:9, :, :], dim=1, keepdim=True)
    output = torch.cat([r, g, b], dim=1)
...
if __name__ == "__main__":
    ...
    metadata = '/data/share/trojai/trojai-round3-dataset/METADATA.csv'
    ...
    logfile = 'result.txt'
    args.result_filepath = './output'
    args.scratch_dirpath = './scratch'

    for i in range(total):
        model_id = 'id-' + str(i).zfill(8)

        args.model_filepath = '/data/share/trojai/trojai-round3-dataset/{}/model.pt'.format(model_id)
        args.examples_dirpath = '/data/share/trojai/trojai-round3-dataset/{}/clean_example_data/'.format(model_id)

        model_architecture = df.loc[model_id]['model_architecture']
