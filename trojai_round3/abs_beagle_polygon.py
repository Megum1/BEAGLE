...
from synthesis_scanner import center_loss, size_loss, mask_init, delta_init
...
def loss_fn(inner_outputs_b, inner_outputs_a, logits, batch_labels, con_mask, neuron_mask, label_mask, base_label_mask, acc, e, re_epochs, ctask_batch_size):
    ...
    # TODO:
    # position: center loss
    # size: torch.gt, torch.le
    for i in range(ctask_batch_size):
        mask = con_mask[i]
        size_add_loss += size_loss(mask, re_mask_weight, mask_epsilon)
        center_add_loss += center_loss(mask, re_mask_weight)
    
    mask_add_loss = size_add_loss + center_add_loss
    ...
    return loss, vloss1, vloss2, vloss3, relu_loss1, relu_loss2, size_add_loss, center_add_loss, mask_add_loss, logits_loss
...
def reverse_engineer(model_type, model, children, oimages, olabels, weights_file, Troj_Layer, Troj_Neurons, samp_labels, base_labels, Troj_size, re_epochs, num_classes, n_re_imgs_per_label, n_neurons, ctask_batch_size):
    ...
    # TODO: delta, mask initialization
    delta = np.tile(delta_init().reshape((1,3,1,1)), (ctask_batch_size,1,1,1))
    delta = torch.FloatTensor(delta).cuda()

    mask = np.tile(mask_init(h, w).reshape((1,1,h,w)), (ctask_batch_size,1,1,1)) * 6 - 4
...
def pixel_check(model_filepath, result_filepath, scratch_dirpath, examples_dirpath, example_img_format='png'):
    ...
    with open(logfile, 'a') as f:
        f.write('mask {0} {1} {2} {3}\n'.format(model_filepath, output, max_reasr, round(optm_end - start, 3)))


def main(args, example_img_format='png'):
