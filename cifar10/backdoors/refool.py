...
class Refool:
    ...
    def init_ghost(self, trigger):
        max_image_size = 560
        offset = (0, 0)
        ghost_alpha = 0.65 # -1.0
        self.alpha_i = 0.65 # -1.0
    ...
    def inject(self, inputs):
        ...
        if self.mode == 'smooth':
            ...
            inputs = self.inject_shallow(inputs)
        return inputs
    ...
    def inject_shallow(self, inputs):
        inputs = torch.clamp((1 - self.alpha) * inputs + self.alpha * self.trigger.to(inputs.device), 0.0, 1.0)
        return inputs
