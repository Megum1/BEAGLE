...
def forensic(args, preeval=True):
    ...
    # Remember to provide some clean and poisoned samples
    # Poison source images do not need to be the same as clean image
    test_set = get_dataset(args.dataset, train=False)

    # Backdoor
    backdoor = get_backdoor(args.attack, test_set[0][0].shape[-1], DEVICE)

    x_sample, y_sample = [], []
    count = args.n_clean + args.n_poison
    for (image, label) in test_set:
        if label == args.target:
            continue
        x_sample.append(image)
        y_sample.append(label)
        if len(x_sample) == count:
            break
    x_sample, y_sample = torch.stack(x_sample), torch.tensor(y_sample)
    x_clean, y_clean = x_sample[:args.n_clean], y_sample[:args.n_clean]
    x_poison_src, y_poison_src = x_sample[args.n_clean:], y_sample[args.n_clean:]
    print(f'Clean samples: {x_clean.shape}, Poison samples: {x_poison_src.shape}')

    # Batch size
    batch_size = x_poison_src.shape[0]
    ...
    if preeval:
        with torch.no_grad():
            n_sample = 0
            n_acc, n_asr = 0, 0
            batches = load_batch(x_clean, y_clean, x_poison_src, y_poison_src)
            for (bx_clean, by_clean, bx_poison_src, by_poison_src) in batches:
                bx_clean, by_clean = bx_clean.to(DEVICE), by_clean.to(DEVICE)
                bx_poison_src, by_poison_src = bx_poison_src.to(DEVICE), by_poison_src.to(DEVICE)

                bx_poison = backdoor.inject(bx_poison_src)
                by_poison = by_poison_src * 0 + args.target

                out_clean, out_poison = model(preprocess(bx_clean)), model(preprocess(bx_poison))
    ...
    steps = args.epochs * (x_clean.shape[0] // x_poison_src.shape[0])
    step = 0
    ...
    with torch.no_grad():
        ...
        x_poison_poi = backdoor.inject(x_poison_src).to(DEVICE)
    ...
    for epoch in range(args.epochs):
        ...
        # Save path
        save_path = f'{args.save_folder}/{args.func}_{args.func_option}_{args.attack}_{args.dataset}_{args.network}'
...
if __name__ == '__main__':
    ...
    parser.add_argument('--n_clean', type=int, default=100, help='number of clean samples')
    parser.add_argument('--n_poison', type=int, default=10, help='number of poison samples')
    ...
    parser.add_argument('--save_folder', default='forensics', help='save folder')
